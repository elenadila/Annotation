...
def Get_DB_Handler():
    return Database_Handler("ADDRESS", 3306, "USERNAME", "PASSWORD", "DBNAME")
...
class Database_Handler:
    ...
    def insert(self, table_name, fields, values):
        ...
        con = pymysql.connect(host=self._database_host, port=self._database_port, user=self._database_user,
                              passwd=self._database_user_password, db=self._database_name)
        con.autocommit(True)
        cursor = con.cursor()
        id = 0
        try:
    ...
    def getGreatestTimestamp(self, table_name, device_id):
        ...
        selectString = "SELECT timestamp FROM %s WHERE device_id = '%s' ORDER BY id DESC LIMIT 1" % (
